...
// src/app/(account)/account/page.tsx (04 §B.2.10, C-10) — the signed-in customer's own
// profile: edit name/phone (email read-only/immutable) and change password, plus logout.
// GET /profile, PATCH /profile ({ name?, phone? }), PATCH /profile/password
//   ({ current_password, new_password }).
// 422 → per-field errors via applyServerErrors; 401 INVALID_CREDENTIALS → field error on
// current_password. Success invalidates ['profile'] + ['auth','me']. Logout clears the
// token + query cache (useAuth) and redirects home.
...
function ChangePasswordForm() {
  ...
  const onSubmit = (values: ChangePasswordValues) => {
    changePassword.mutate(values, {
      ...
      onError: (err) => {
        // 401 INVALID_CREDENTIALS → wrong current password (§B.2.10).

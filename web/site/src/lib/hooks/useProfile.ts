...
// src/lib/hooks/useProfile.ts (04 §4) — React Query hooks for the signed-in customer's
// own profile (C-10). Mutations invalidate ['profile'] and ['auth','me'] per the §4.3
// invalidation matrix.
import {
  useMutation,
  useQuery,
...
> {
  ...
  return useMutation<UserDTO, ApiError, UpdateProfileRequest>({
    ...
    onSuccess: (updated) => {
      ...
      qc.setQueryData(qk.profile.detail(), updated);

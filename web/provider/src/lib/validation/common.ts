...
// Feature schemas (login, profile, availability, reject reason, etc.) compose these
// primitives. Server remains the source of truth on 422.
import { z } from 'zod';
...
export function applyServerErrors(
  ...
  if (err.code === 'VALIDATION_ERROR' && err.details?.length) {
    ...
    // DUPLICATE_RESOURCE, INVALID_STATUS_TRANSITION, CONFLICT, etc.

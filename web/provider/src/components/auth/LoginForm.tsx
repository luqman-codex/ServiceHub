...
// src/components/auth/LoginForm.tsx (04 §A.2.1, §9) — provider login form.
// RHF + zod; calls useAuth().login; rejects non-PROVIDER at the UI layer; maps 422
...
export function LoginForm() {
  ...
  const mutation = useMutation<AuthResultDTO, ApiError, LoginValues>({
    ...
    onSuccess: (result) => {
      // Provider portal rejects non-PROVIDER logins at the UI layer (§A.2.1).
      if (result.user.role !== 'PROVIDER') {
        tokenStore.clear();
        setFormError('This portal is for service providers only.');
        return;
      }
      ...
      <FormField name="email" label="Email" required>
        {(p) => (
          <Input
            type="email"
            autoComplete="email"
            placeholder="provider@servicehub.com"
            invalid={p.invalid}
            id={p.id}
